# igp-mesh
Repository to track the work on creating a mesh group using TLVs that are advertised as part of IGP.
# What do you need
To build the draft in this repository, you need to have Docker installed and running on the machine where this repository will be cloned.
...
- draft
- draft/Makefile
- draft/draft-admnr-lsr-igp-measurement-group.xml
...
Create a branch off of the 'master' branch that matches the version of the draft to be published. For example, if the next version of the branch is draft-admnr-lsr-igp-measurement-group-01, create a branch for v01 using the following command:
...
However, keep this branch "clean", as in do not make changes in the branch directly. Instead, allow PR to update the branch. To do that, create "issue branches" off of the new branch you have created. For example, if you create an issue #1, to update the name of the draft in the XML file, create a branch issue-01 and do your work there.
...
Next, create a git tag for the draft-admnr-lsr-igp-measurement-group.xml as follows:

	`git tag draft-admnr-lsr-igp-measurement-group-00`

This will tell 'make' to build the draft with the next version. In this case, since the tag is for -00, it will build -01.
...
As mentioned already, the 'git tag' command tells make what version to build. To do that right, the name of the draft in the XML file (docName under &lt;rfc&gt; and value under &lt;seriesInfo&gt if that line exists;) should be called the name of the draft followed by the word 'latest'. For example, if the draft name is draft-admnr-lsr-igp-measurement-group, then both the variables should say 'draft-admnr-lsr-igp-measurement-group-latest'. Make will replace latest with the version that the draft needs to be built with.
